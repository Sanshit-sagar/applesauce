...
const ColorPicker = ({ editor }: { editor: Editor }) => (
    <>
        ...
        <button onClick={() => editor.chain().focus().setColor('#f98181').run()}>
            red
        </button>
...
const CodeBlockComponent = ({ editor }: { editor: Editor }) => {

    return (
        <button 
            onClick={() => editor.chain().focus().toggleCodeBlock().run()} className={editor.isActive('codeBlock') ? 'is-active' : ''}>
        code block
        </button>
    )
}
