...
const sanitizedStatusSuffix = (editor: Editor) => user${(editor?.storage?.collaborationCursor?.users?.length ?? 0) === 1 ? '' : 's'} online in ${room}`;
...
const Tiptap = () => {
    ...
    return (
        <TiptapEditor>
            ...
            <EditorFooter>
                <Status> 
                    {   `${numActiveUsers(editor)} ${sanitizedStatusSuffix(editor)} 
                    }
                </Status>
