...
const userActivityStatus = (editor: Editor, room: number) => `${numActiveUsers(editor)} ${statusSuffix(editor, room)}`
const statusSuffix = (editor: Editor, room: number) => `user${(editor?.storage?.collaborationCursor?.users?.length ?? 0) === 1 ? '' : 's'} online in ${room}`;
    ...
    return (
        <TiptapEditor>
            ...
            <EditorFooter>
                ...
                <ActiveUser onClick={setName}> {currentUser ? currentUser.name : ''} 
                </ActiveUser>
            </EditorFooter>
